...
import { TasksState } from './task.state.interface';
...
export const taskReducer = createReducer(
  ...
  on(TaskActions.deleteTaskSuccess, (state, action) => ({
    ...
    tasks: state.tasks.filter((task) => task._id !== action.taskId), // Remove the deleted task from the tasks array
  })),
  ...
  on(TaskActions.addTaskSuccess, (state, action) => ({
    ...
    tasks: [...state.tasks, action.task], // Add the newTask to the tasks array
  })),
